# Dashboard Plugin v2.13.8

Dashboard Plugin v2.13.8 is a standalone WordPress shortcode plugin for creating multiple live dashboard metrics from published Google Sheets.
...
- v2.13.3 memoises normalized settings during each request, throttles repeated resize recalculations, and keeps the admin version label and release link in sync.
- v2.13.4 avoids fetching Google Sheets during public page rendering. The page can be served immediately from LiteSpeed/page cache, while the uncached frontend REST refresh fetches the current Sheet value after load.
- v2.13.5 batches the frontend refresh into one uncached request and fetches each unique published Sheet/tab once, then renders all dashboard values from that fresh response.
- v2.13.6 displays an accessible loading state while the live refresh is waiting, so the page does not appear to be stalled or show an error prematurely.
- v2.13.7 stores the last successful value for up to one hour as a stale display fallback. Every refresh still requests Google Sheets, and a fresh response always replaces the stored value.
- v2.13.8 pre-warms all configured Sheet values in the background every ten minutes and after settings changes, so the first public page load can normally display a recent value immediately. The browser still performs its own live refresh on every page load.
...
3. Upload the stable update package for the v2.13.8 release.
...
Download: `https://github.com/andyhayes74-commits/dashboard-plugin/raw/refs/heads/v2.13.8-first-load/dashboard-plugin-main-v2.13.8.zip`
...
- **Mobile layout** — mobile text size, value size, line spacing, inner padding, maximum width, and graphic height. These values apply up to 767px wide and fall back to the normal settings when blank.
- **Widget appearance** — theme preset, card preset, border style, background treatment, decorative graphic, animated graphic, and graphic maximum.
...
- Public page rendering does not wait for Google Sheets. Background warm-up keeps recent results available for the first visitor, while the uncached frontend REST refresh still fetches the current value after load. The last successful result is retained for up to one hour as a stale display fallback only.
